...
public class UnderWaterProgram extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_underprogram);
        ...
        bar.setTitle("수중재활센터");   // 액션바 타이틀 라벨

        // 액션바에 모드 설정 = ActionBar.NAVIGATION_MODE_TABS 로 TAB 모드로 설정
        bar.setNavigationMode(ActionBar.NAVIGATION_MODE_TABS);

        // 액션바에 추가될 탭 생성
        ActionBar.Tab tab1 = bar.newTab().setText("수중재활운동").setTabListener(tabListener);
        ActionBar.Tab tab2 = bar.newTab().setText("수중운동,특수수영").setTabListener(tabListener);
        ActionBar.Tab tab3 = bar.newTab().setText("방과후,가족지원").setTabListener(tabListener);
